...
* Unit-00: ScaleTableMatrix (64 x 13-bit words paired as 32 x 26-bit)
* Unit-01: IDCT intermediate result (16 x 13-bit words, separate IO)
* Unit-10: MDEC output RGB macroblock (?) (16x16 24-bit RGB values, 768 bytes total)
* Unit-26: D-Cache (1 KByte)
* Unit-30: I-Cache (4 KBytes)

[[File:cpu_units.jpg|500px]]
...
[[File:cpu_unit_type1.jpg|800px]]

Список блоков:
* Unit-00: ScaleTableMatrix 8x8 (32 registers per 26 bits) used as 64 records per 13bits.
* Unit-01: IDCT intermediate result (13 vertical, 16 horizontal)
...
[[File:Unit00 cell.jpg]]

Обычная би-стабильная защелка на базе двух инверторов и управляющие транзисторы:
...
5й бит индекса ряда не используется (подключен к земле), так как у нас адресуется только 32 ряда ячеек (биты индекса 0-4).
...
Схема комбинирует 5й разряд и входной контрольный сигнал input enable, для "подключения" верхней или нижней половины. Output enable контроль однако не предусмотрен и контролируется только декодером (row select).
...
[[File:Unit00 pads.jpg|1000px]]

==== Unit-01: Промежуточный результат вычислений IDCT ====

[[File:Unit01 overview.jpg|640px]]
...
[[File:Unit24 overview.jpg|600px]]
...
[[File:Unit24 io enable.jpg|300px]]
...
[[File:Unit24 reg inputs outputs.jpg|200px]]

==== 2-to-4 decoders ====

[[File:Uni24 decoder 2-to-4.jpg|542px]]

Часть декодера.

==== ANDs ====

[[File:Unit24 ands.jpg]]
...
Блоки четверного типа представляют собой статическую память SRAM, использующуюся как кеш для различных данных.

[[File:cpu_unit_type4.jpg|800px]]
...
Кеш данных имеет на входе 18 управляющих линий (среди них где-то линии шины адреса), а также 32x2 линии Data bus.

Кеш инструкций имеет 17 управляющих линий (2 из них не используются) и 32x2 шину данных (инструкцию).
...
[[File:DCache data lines.jpg]] [[File:ICache data lines.jpg]]
...
[[File:Sram cell trans.jpg]]

Элемент памяти - это latch, основанный на спаренных инверторах, входы которых подсоединены к column line через row line tri-state буфер.
...
Управлением доступа к ячейкам занимается специальный RAS/CAS декодер, которые на базе входной шины адреса формирует линии row/column.
